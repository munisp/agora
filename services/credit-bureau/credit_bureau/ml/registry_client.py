...
Resolves the tenant's PRODUCTION ``credit-ml`` artifact through the
...
registry. The contract (coded against the service spec, not its code)::
    ...
    GET {MODEL_REGISTRY_URL}/v1/registry/credit-ml/{tenant_id}/production
    200 -> {"family": "credit-ml", "tenant_id": ..., "version": ...,
...
must be ``credit-ml``, ``tenant_id`` must equal the requested tenant,
...
log = logging.getLogger("credit_bureau.ml.registry_client")
