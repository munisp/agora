...
def _person_row(p: Any) -> dict[str, Any]:
    return {
        ...
        # W29 predictive scores + W30 fraud risk head (null when unscored).
        "propensity_churn": p.props.get("propensity_churn"),
        "propensity_convert": p.props.get("propensity_convert"),
        "propensity_turnout": p.props.get("propensity_turnout"),
        "risk_score": p.props.get("risk_score"),
        "risk_flags": p.props.get("risk_flags") or [],
...
def _render_person_by_id(norm: dict[str, Any], cap: int) -> tuple[str, dict[str, Any]]:
    cypher = (
        ...
        "       p.propensity_convert AS propensity_convert,\n"
        "       p.propensity_turnout AS propensity_turnout,\n"
        "       p.risk_score AS risk_score,\n"
        "       coalesce(p.risk_flags, []) AS risk_flags,\n"
        "       p.model_version AS model_version, p.scored_at AS scored_at\n"
        "LIMIT 1"
    )
...
# SPEC-W29 §3 WS-B / SPEC-W30 §4 WS-C: predictive + fraud-alert templates
# join the same read-only allowlist (imported at the END so the submodules
# can reuse Template/GraphView/helpers defined above without a cycle).
from . import alerts as _alerts  # noqa: E402
from . import predictive as _predictive  # noqa: E402

TEMPLATES.update(_predictive.PREDICTIVE_TEMPLATES)
TEMPLATES.update(_alerts.ALERT_TEMPLATES)
