...
from ..plans import CompiledQuery, PersonFilterPlan, ScoreFilterSpec
...
def compile_segment_query(
    ...
    *,
    ...
    # SPEC-W29 §3 WS-B: numeric score filters. Every bound is a parameter
    # ($sf0, $sf1, ...) — values are NEVER interpolated into the Cypher text.
    # The DSL model (dsl.ScoreFilter) already rejected unknown fields with a
    # validation error (-> 422); the guard below is defense-in-depth for
    # programmatic callers.
    score_specs: list[ScoreFilterSpec] = []
    sf_index = 0
    for sf in f.score_filters:
        field = sf.field
        if sf.op == "between":
            lo, hi = float(sf.value[0]), float(sf.value[1])  # type: ignore[index]
            lo_param, hi_param = f"sf{sf_index}", f"sf{sf_index + 1}"
            sf_index += 2
            params[lo_param] = lo
            params[hi_param] = hi
            conditions.append(f"p.{field} >= ${lo_param} AND p.{field} <= ${hi_param}")
            score_specs.append(ScoreFilterSpec(field=field, op="between", lo=lo, hi=hi))
        else:
            bound_param = f"sf{sf_index}"
            sf_index += 1
            bound = float(sf.value)  # type: ignore[arg-type]
            params[bound_param] = bound
            conditions.append(f"p.{field} {sf.op} ${bound_param}")
            score_specs.append(ScoreFilterSpec(field=field, op=sf.op, lo=bound))
    if score_specs:
        plan_kwargs["score_filters"] = tuple(score_specs)
