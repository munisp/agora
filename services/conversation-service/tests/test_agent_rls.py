...
async def test_empty_string_tenant_guc_denies_without_error(stores):
    """W40-6: a recycled asyncpg pool connection may carry app.tenant_id=''
    (EMPTY string, not NULL). A bare current_setting('app.tenant_id', true) uuid-cast qual
    ...
    every tenant table returns 0 rows WITHOUT raising.
    """
    _, subject, _, subject_pool = stores
    await subject.create_agent(TENANT_A, "EmptyGuc A")
    await subject.create_agent(TENANT_B, "EmptyGuc B")
    async with subject_pool.acquire() as conn:
