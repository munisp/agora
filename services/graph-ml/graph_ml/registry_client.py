...
Resolves the tenant's PRODUCTION ``graphsage`` artifact through the
...
registry. The contract (coded against the service spec, not its code)::
    ...
    GET {MODEL_REGISTRY_URL}/v1/registry/graphsage/{tenant_id}/production
    200 -> {"family": "graphsage", "tenant_id": ..., "version": ...,
...
must be ``graphsage``, ``tenant_id`` must equal the requested tenant,
...
log = logging.getLogger("graph_ml.registry_client")
