...
func TestTenantDeleted_DetachesWholeSubgraph_Idempotent(t *testing.T) {
	g := newFakeGraph()
	s, _, _ := newTestSyncer(g)
	ctx := context.Background()

	// Seed a tenant subgraph (booking path anchors Tenant + Person + Booking).
	require.NoError(t, s.HandleBooking(ctx, bookingEvent("evt-seed", "tenant-gone", map[string]any{
		"booking_id": "bk-1", "contact_id": "ct-1", "phone": "+2348030000000", "status": "pending",
	})))
	require.Len(t, g.persons, 1)
	_, ok := g.tenants["tenant-gone"]
	require.True(t, ok)
	...
	evt := events.CloudEvent{
		...
		Data: map[string]any{
			"tenant_slug": "acme-gone",
			"tenant_id":   "tenant-gone",
			"deleted_at":  "2026-08-05T11:00:00Z",
			"actor":       "platform-admin",
		},
	}
	require.NoError(t, s.HandleIdentity(ctx, evt))
	require.Empty(t, g.persons, "person nodes deleted with the tenant")
	require.Empty(t, g.bookings)
	_, ok = g.tenants["tenant-gone"]
	require.False(t, ok, "tenant anchor deleted")
	require.Contains(t, s.Metrics.Render(), `graph_sync_counter{name="tenant_subgraphs_deleted"} 1`)

	// Redelivery with a fresh event id (marker was deleted with the
	// subgraph): the delete itself is idempotent — second run is a no-op.
	evt.ID = "evt-del-2"
	require.NoError(t, s.HandleIdentity(ctx, evt))
	require.Empty(t, g.persons)
}
