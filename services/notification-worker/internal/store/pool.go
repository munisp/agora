...
// SPEC-W46 PERF-08: budgeted pgx pools. pgx defaults (MaxConns=
...
//
//	notification-worker (this process): main 10 + internal 4 = 14 conns/replica
//	identity-service: 38 · kyc 10 · crm-sync 10 · messaging-gateway 0
...
const (
	// poolMaxConns is the per-pool default for the tenant-scoped pool.
	poolMaxConns int32 = 10
	// poolInternalMaxConns is the default for the internal/escape-role
	// pool (suppression checks, outbox sweeps, id-keyed updates).
	poolInternalMaxConns int32 = 4
	// poolMinConns keeps two warm connections (no cold-connect latency
	// on first queries after idle).
	poolMinConns int32 = 2
	// poolInternalMinConns keeps one warm connection on the internal pool.
	poolInternalMinConns int32 = 1
