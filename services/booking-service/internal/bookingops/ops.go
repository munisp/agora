...
type Service struct {
	...
	Cache *cache.Cache
	// Payments is the payments-rail client for the K11/K12 money loop
	// (deposit capture on complete, refunds). Nil = PAYMENTS_URL unset —
	// money-moving endpoints fail closed with ErrPaymentsNotConfigured.
	Payments *PaymentsClient
	// Loyalty is the K11 accrual hook (BookingCompletedAccruer). Nil when
	// the loyalty package does not expose AccrueOnBookingCompleted yet —
	// see the CONTRACT NOTE on the interface.
...
func (s *Service) Create(ctx context.Context, in CreateInput) (store.Booking, error) {
	...
	// Resolve or create the contact. Phone-presence policy (SPEC §1/§11,
	// corrected SPEC-W45 STK O8 — the old comment claimed a VERIFIED phone,
	// which the platform does not track): mutations are rejected without a
	// contact phone. Presence is all that is enforced here; phone
	// verification is not implemented anywhere in the stack.
	...
	case in.Contact != nil:
		...
		// SPEC-W45 CODER-A item 5: match-or-create on (tenant_id, phone)
		// (uq_contacts_tenant_phone) instead of blindly inserting a
		// duplicate contact per booking — mirrors UpsertExternalContact.
		...
		if _, err := s.Store.UpsertBookingContact(ctx, in.TenantID, &contact); err != nil {
			return store.Booking{}, err
		}
	default:
