...
# --------------------------------------------------------------------------
# SPEC-W45 K15(c) / OOS-03: the carrier-asserted pre-confirmation bypass is
# REMOVED — the SIP caller ID is a CLAIMED (unverified) hint only.
# --------------------------------------------------------------------------
class TestCallerIdClaimed:
    def test_attach_sets_claimed_not_confirmed(self):
        session = SessionState(conversation_id="c1", site_slug="acme")
        assert sip.attach_caller_id(session, "+1 (555) 222-3333")
        assert session.claimed_phone == "+15552223333"
        # The bypass is gone: confirmed/verified stay unset.
        assert session.confirmed_phone is None
        assert session.verified_phone is None

    def test_no_bypass_two_step_confirmation_still_applies(self):
        """A SIP session with an attached caller ID faces the SAME
        confirmation policy as web — no carrier-asserted shortcut."""
        session = SessionState(conversation_id="c1", site_slug="acme")
        sip.attach_caller_id(session, "+15552223333")
        # No phone argument: nothing is confirmed -> confirmation required.
        with pytest.raises(PhoneConfirmationRequired):
            session.require_confirmed_phone(None)
        # Even the claimed number itself must go through the two-step flow.
        with pytest.raises(PhoneConfirmationRequired):
            session.require_confirmed_phone("+15552223333")
        # Second call with the same number completes the read-back.
    ...
    def test_anonymous_caller_keeps_normal_policy(self):
        ...
        assert session.claimed_phone is None
        with pytest.raises(PhoneConfirmationRequired):
            session.require_confirmed_phone("+15559990000")

    def test_attach_does_not_verify(self):
        """Claimed identity never satisfies the K15(c) OTP gate."""
        session = SessionState(conversation_id="c1", site_slug="acme")
        sip.attach_caller_id(session, "+15552223333")
        assert session.verified_phone is None
...
class TestBootstrap:
    def test_full_inbound_bootstrap(self):
        ...
        # K15(c): claimed (unverified) — never confirmed.
        assert session.claimed_phone == "+15552223333"
        assert session.confirmed_phone is None
