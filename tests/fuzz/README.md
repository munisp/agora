...
The seed corpus is committed in-tree at
`services/crm-sync-service/internal/httpapi/testdata/fuzz/FuzzVerifySignature/`
(one file per `f.Add(...)` seed in `webhook_fuzz_test.go` — this closes the
SPEC-W41 corpus claim, landed in W42). Note: corpus entries *generated by*
a fuzz run (new interesting inputs, any crashers) still land in the build
cache (`GOCACHE/fuzz/...`), not in `testdata/` — promote them into the
in-tree corpus manually if a run finds any.
