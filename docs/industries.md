...
| **travel** | concierge / reservation / guest | 48h cancellation window | `ConsultancyFollowupWorkflow` | Warm concierge persona; group capacities (shuttle 8, city tour 15); `check_flight_status` custom tool REMOVED in W39 (a listed-but-unimplemented tool is a runtime lie); re-added only when the Amadeus plugin endpoint lands, allowlisted via `PLUGIN_ALLOWED_HOSTS` |
| **ecommerce** | associate / slot / customer | 4h cancellation window, capacity-based slots | `SupportEscalationWorkflow` | Order-support persona (verify via order lookup, never guess); `shopify_order_lookup` / `woocommerce_order_lookup` custom tools REMOVED in W39 (the plugin-shopify / plugin-woo sidecars they targeted do not exist anywhere in this repo — a listed-but-unimplemented tool is a runtime lie); re-added only when those sidecars land and are allowlisted via `PLUGIN_ALLOWED_HOSTS`; order-support + sales-assistant crew agents |
...
| **transportation** | travel officer / reservation / passenger | 100% prepayment at ticketing, 24h window | `SupportEscalationWorkflow` | Three-mode travel-operations persona (airline/train/bus); disruption handling with per-mode refund rules; high capacities (bus 50, charter 40, group rail 20); `check_schedule` custom tool REMOVED in W39 (same runtime-lie rationale); re-added only with a live timetables/schedules plugin endpoint plus a `PLUGIN_ALLOWED_HOSTS` allowlist entry; airline-desk + rail-desk + bus-desk crew agents |
